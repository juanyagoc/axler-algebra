...
\]
  Knowing the norm of $u$ is 1 and that $u^*$ represents the complex conjugate which in the real field $\mathbb{R}$ equals the transpose $u^T$ we end up with
\[
...
\end{definition}
\begin{proof}
  Suppose A is fomed with the list $v_1,\ldots ,v_n$ by the Gram-Schmidt prodedure we can write any vector $v$ of the vector space $V$ in the form
  \[
...
H_k \cdots H_1 A = R
\]
and therefore $Q = (H_k \cdots H_1)^{-1} = H_k^* \cdots H_1^*$.
