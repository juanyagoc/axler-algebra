...
Definitions, theorems and exercises from the fourth edition of the book \textit{Linear Algebra Done Right} by Sheldon Axler. As well as some computational linear algebra concepts and tikz tests.
...
% Plantilla para representar arrays y punteros
\begin{center}
\begin{tikzpicture}[every node/.style={draw, minimum size=0.8cm, align=center}]
    \node               (a0) at (0,0)   {1};
    \node[fill=red!50]  (a1) at (0.8,0) {2};
    \node               (a2) at (1.6,0) {3};
    \node[fill=blue!50] (a3) at (2.4,0) {4};
    \node               (a4) at (3.2,0) {5};
    \node               (a5) at (4.0,0) {6};
    \node               (a6) at (4.8,0) {7};
    \draw[->, thick] (0.8,-1) -- (a1.south |- 0,-0.5);
    \draw[->, thick] (2.4,-1) -- (a3.south |- 0,-0.5);
\end{tikzpicture}
\end{center}

\begin{definition}[Hola]
La notación para un homomorfismo $h$ de un lenguaje formal $\Sigma^*$ a otro $\Delta^*$ es la siguiente:
\[
h : \Sigma^* \longrightarrow \Delta^*
\]
Por ejemplo, definiendo de forma recursiva la extensión $h$:
\[
\begin{cases}
    h(\lambda) = \lambda \\
    h(xa) = h(x)h(a)
...
\newpage
\begin{center}
    Ejemplo de árbol Rojo-Negro según el esquema de CLRS

\begin{tikzpicture}[
    every node/.style={circle, draw=black, minimum size=6mm},
    ...
    r/.style={fill=red!75,draw=red,text=white},
    n/.style={fill=black!75,draw=black,text=white},
    nil/.style={draw=none,fill=white,draw=white}
]
\node[n] {5}
child {node[r] {2}
    child {node[n] {1}
...
\begin{tikzpicture}
    ...
    \draw[blue, line width=1.5pt]
        (0:\r)
        \foreach \i in {60,120,180,240,300} {
            -- (\i:\r)
        } -- cycle;
    \foreach \i [count=\n from 0] in {0,60,120,180,240,300} {
        \fill[black] (\i:\r) circle (2.5pt);
        \ifnum\n=0
            \node at (\i:\r + 2.7cm) {$Z^{\n}=1$};
        \else
            \node at (\i:\r + 2.4cm) {$Z^{\n}$};
